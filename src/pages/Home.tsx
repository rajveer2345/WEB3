...
// import { LOTTERY_ABI, LOTTERY_CONTRACT_ADDRESS, USDT_ABI, USDT_CONTRACT_ADDRESS } from '../contract/details'
...
// const wagmiContractConfig: any = { abi: LOTTERY_ABI, address: LOTTERY_CONTRACT_ADDRESS };
...
//         // args: ['0x03A71968491d55603FFe1b11A9e23eF013f75bCF']
//     })
//     const { data: lottertOperator } = useReadContract({
//         ...wagmiContractConfig,
...
//         // args: ['0x03A71968491d55603FFe1b11A9e23eF013f75bCF']
//     })
//     const { data: getTickets } = useReadContract({
//         ...wagmiContractConfig,
...
//         try {
//             // 1. Correct: use readContract instead of useReadContract
...
//             setTxHash(buyTx);
//             const receipt = await waitForTransactionReceipt(config, { hash: buyTx, });
...
//             const receipt = await waitForTransactionReceipt(config, { hash: withdrawTx, });
...
//         } catch (error) {
//             console.error('Withdraw commission failed:', error);
//         } finally {
//             setLoading(false);
//         }
//     };
...
//             setTxHash(refundTx);
//             const receipt = await waitForTransactionReceipt(config, { hash: refundTx, });
...
//         console.log(operatorTotalCommission, "hi hi")
...
//                     {loading && <div className='bg-black/50 fixed inset-0 top-0 z-50 flex justify-center items-center'><span className="loader"></span></div>}
//                     {/* navbar */}
...
//                         </Marquee>
//                         {winnings > 0 && <WinnerCard winnings={winnings} withdrawWinnings={withdrawWinnings} />}




//                         <Test remainingTickets={ticketsLeft} expiration={expiration} CurrentWinningReward={CurrentWinningReward} />
//                         {address == lottertOperator && <AdminControls operatorTotalCommission={operatorTotalCommission} withdrawCommission={withdrawCommission} drawWinner={drawWinner} restartDraw={restartDraw} refundAll={refundAll} />}
//                         <Test2 getTickets={getTickets} address={address} buyTickets={buyTickets} />
...
const Home = () => {
  ...
  const handleTransaction = useCallback(async (operation, actionFunction) => {
    if (!isConnected) {
    ...
    try {
      ...
      toast.dismiss(loadingToastId);
      toast.error(error?.cause?.message || `${operation} failed. Please try again.`);
    } finally {
      setLoading(false);
    }
  }, [isConnected]);
